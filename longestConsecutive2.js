...
var longestConsecutive = function(nums) {
  ...
  for (var iter = 0, len = nums.length; iter < len; iter++) {
    ...
    if (hashmap[nums[iter]+1] !== undefined) { hashmap[nums[iter]] += hashmap[nums[iter]+1]; console.log("num " + nums[iter] + " " + hashmap[nums[iter]]);}
    ...
    while(hashmap[prevKey]) {
      ...
      console.log("num " + nums[iter] + " " + hashmap[nums[iter]]);
